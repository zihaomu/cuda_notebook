...
__global__ void gemm_v03(size_t m, size_t n, size_t k, T *A, T *B, T *C, T alpha, T beta)
{
    ...
    size_t const inner_y_b = threadIdx.x/X_tile; // 8
    size_t const inner_x_b = threadIdx.x - inner_y_b * X_tile; // 64

    size_t const inner_y_a = threadIdx.x/Y_num; // 64
    ...
    for (int i = 0; i < loop_count; i++)
    {
        ...
        for (size_t ii = 0; ii < K_tile; ++ii)
        {
            ...
            {
                sum[j] += A_tile[inner_y_b * Y_num + j][ii] * b_val;
            }
        }
...
void cuda_gemm_v3(size_t m, size_t n, size_t k, T *A, T *B, T *C, T alpha, T beta, cudaStream_t stream)
{
    ...
    gemm_v03<T, X_tile, Y_tile, K_tile, Y_num, NUM_THREAD><<<grid_dim, block_dim, 0U, stream>>>(m, n, k, A, B, C, alpha, beta);
    CHECK_LAST_CUDA_ERROR();
}


template GEMM_EXPORT void cuda_gemm_v3<float>(size_t m, size_t n, size_t k, float *A, float *B, float *C, float alpha, float beta, cudaStream_t stream);
template GEMM_EXPORT void cuda_gemm_v3<__half>(size_t m, size_t n, size_t k, __half* A, __half* B, __half* C, __half alpha, __half beta, cudaStream_t stream);
