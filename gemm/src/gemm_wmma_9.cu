...
static __global__ void gemm_wmma_kernel_fp16_v9(size_t M, size_t N, size_t K, 
...
template <>
void cuda_gemm_wmma_9<__half>(size_t M, size_t N, size_t K,
    __half *A, __half *B, __half *C, 
    ...
    CUDA_CHECK(cudaFuncSetAttribute(gemm_wmma_kernel_fp16_v9,
    ...
    gemm_wmma_kernel_fp16_v9<<<gridDim, blockDim, value, stream>>>(M, N, K, A, B, C, alpha, beta);
    CHECK_LAST_CUDA_ERROR();
}

template <>
void cuda_gemm_wmma_9<float>(size_t m, size_t n, size_t k, float* A, float* B, float* C, float alpha, float beta, cudaStream_t stream)
{
...
template GEMM_EXPORT void cuda_gemm_wmma_9<float>(size_t m, size_t n, size_t k, float *A, float *B, float *C, float alpha, float beta, cudaStream_t stream);
template GEMM_EXPORT void cuda_gemm_wmma_9<__half>(size_t m, size_t n, size_t k, __half* A, __half* B, __half* C, __half alpha, __half beta, cudaStream_t stream);
