...
__global__ void gemm_v07(size_t m, size_t n, size_t k, T *A, T *B, T *C, T alpha, T beta)
{
    ...
    for (int i = 0; i < loop_count; i++)
    {
        ...
        if (w1 < n && i * K_tile < k)
        {
            B_tile[inner_y_b][inner_x_b] = B[(i * K_tile + inner_y_b) * n + w1]; // 这种没有考虑warp优化的情况。
        }
        else
        {
...
void cuda_gemm_v7(size_t m, size_t n, size_t k, T *A, T *B, T *C, T alpha, T beta, cudaStream_t stream)
{
    ...
    gemm_v07<T, X_tile, Y_tile, K_tile, Y_num, NUM_THREAD><<<grid_dim, block_dim, 0U, stream>>>(m, n, k, A, B, C, alpha, beta);
    CHECK_LAST_CUDA_ERROR();
}


template GEMM_EXPORT void cuda_gemm_v7<float>(size_t m, size_t n, size_t k, float *A, float *B, float *C, float alpha, float beta, cudaStream_t stream);
template GEMM_EXPORT void cuda_gemm_v7<__half>(size_t m, size_t n, size_t k, __half* A, __half* B, __half* C, __half alpha, __half beta, cudaStream_t stream);
