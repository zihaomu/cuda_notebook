...
优化思路：
双缓冲要求两倍的A和B缓存，如果按照128x128的块划分，则需要128x128x2x2 x size_of(half)= 132KB，超过shared memory的大小。

将A和B减少到64x64的块，这样就只需要64x64x2x2 x size_of(half) = 33KB，刚好小于shared memory的大小。

优化后的结果，性能下降，经过分析，存在更多的bank conflict，以及更低的内存利用率。
...
{
    int* src = (int*)(_src + threadIdx.y * src_ldm * 2 + threadIdx.x * 2);
    int* dst = (int*)(_dst + threadIdx.y * dst_ldm * 2 + threadIdx.x * 2);
...
static __global__ void gemm_wmma_kernel_fp16_v5(size_t M, size_t N, size_t K, 
...
template <>
void cuda_gemm_wmma_5<__half>(size_t M, size_t N, size_t K,
    __half *A, __half *B, __half *C, 
    ...
    CUDA_CHECK(cudaFuncSetAttribute(gemm_wmma_kernel_fp16_v5,
    ...
    gemm_wmma_kernel_fp16_v5<<<gridDim, blockDim, value, stream>>>(M, N, K, A, B, C, alpha, beta);
    CHECK_LAST_CUDA_ERROR();
}

template <>
void cuda_gemm_wmma_5<float>(size_t m, size_t n, size_t k, float* A, float* B, float* C, float alpha, float beta, cudaStream_t stream)
{
...
template GEMM_EXPORT void cuda_gemm_wmma_5<float>(size_t m, size_t n, size_t k, float *A, float *B, float *C, float alpha, float beta, cudaStream_t stream);
template GEMM_EXPORT void cuda_gemm_wmma_5<__half>(size_t m, size_t n, size_t k, __half* A, __half* B, __half* C, __half alpha, __half beta, cudaStream_t stream);
