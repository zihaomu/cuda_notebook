...
同时，当前版本的bank conflict数量和上一个版本的数量也差不多，说明bank的提升有限。

compute 结果：
相对上个版本，SM吞吐量增加，但是内存吞吐量下降。
内存对速度的影响更为重要。
*/
...
static __device__ void load_AB(half* _src, half* _dst, int src_ldm, int dst_ldm)
{
    ...
    for (int i = 0; i < BLOCK_SIZE_DIV8; i++)
    {
        ...
        *(dst + i * dst_ldm) = *(src + i * src_ldm);
    }
}

static inline __device__ void load_A_and_B(half* _A, half* _B, half* As, half* Bs, int K, int N)
{
    ...
    int* src_i = (int*)(src + loadAB_2 * BLOCK_SIZE_DIV2 * k_strip);
    int* dst_i = (int*)(dst + loadAB_2 * BLOCK_SIZE_DIV2 * BLOCK_SKEW_SIZE);

    for (int i = 0; i < BLOCK_SIZE_DIV2; i++)
    {
        int tid = threadIdx.x ^((( i&4)<<4));
        *(dst_i + i * BLOCK_SKEW_SIZE_DIV2 + tid ) = src_i[i * k_strip_DIV2 + tid];
    }
}
...
template <>
void cuda_gemm_wmma_6<__half>(size_t M, size_t N, size_t K,
    __half *A, __half *B, __half *C, 
...
template <>
void cuda_gemm_wmma_6<float>(size_t m, size_t n, size_t k, float* A, float* B, float* C, float alpha, float beta, cudaStream_t stream)
{
...
template GEMM_EXPORT void cuda_gemm_wmma_6<float>(size_t m, size_t n, size_t k, float *A, float *B, float *C, float alpha, float beta, cudaStream_t stream);
template GEMM_EXPORT void cuda_gemm_wmma_6<__half>(size_t m, size_t n, size_t k, __half* A, __half* B, __half* C, __half alpha, __half beta, cudaStream_t stream);
