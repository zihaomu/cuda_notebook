...
from triton_layer import triton_matmul_fp32, triton_matmul_add_fp32
...
class Linear:
    ...
    def load_weights(self, weights, bias=None):
        ...
        :return:
        '''
        assert self.weightsT is None, "linear not initialized"
        # self.weightsT is 
        self.weightsT = weights.T.to(device)
        if self.has_bias:
    ...
    def forward(self, x):
        ...
        # compute the time to convert float32 to float16
        # x = x.half()
        # print(f"convert x to float16 time: {end_time - start_time:.6f} seconds")
        # 检查维度是否正确
        # TODO 使用triton实现linear
        assert self.weightsT is not None, "linear not initialized"
        # Linear: torch.Size([1, 1, 1536]), torch.Size([1536, 1536]), torch.Size([1536])
        # Linear: torch.Size([1, 1, 1536]), torch.Size([1536, 256]), torch.Size([256])
        # x shape = [batch_size, seq_len, in_features], seq_len is 1.
        # weights shape = [out_features, in_features],
        if self.has_bias:
            # print(f"Linear: {x.shape}, {self.weightsT.shape}, {self.bias.shape}")
            # use torch matmul instead of np.matmul
            return triton_matmul_add_fp32(x, self.weightsT, self.bias)
            # return torch.matmul(x, self.weightsT) + self.bias
            # return np.matmul(x, self.weights.T) + self.bias
        else:
            # Linear: torch.Size([1, 1, 8960]), torch.Size([8960, 1536])
            # Linear: torch.Size([1, 1, 1536]), torch.Size([1536, 1536])
            # print(f"Linear: {x.shape}, {self.weightsT.shape}")
            # return torch.matmul(x, self.weightsT)
            return triton_matmul_fp32(x, self.weightsT)
...
class Qwen2RMSNorm:
    ...
    def load_weights(self, weight):
        ...
        self.weight = weight.to(device)  # 将权重移动到GPU上
