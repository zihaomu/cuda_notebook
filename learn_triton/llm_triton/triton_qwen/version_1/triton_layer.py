...
def matmul_1xK_KxN_kernel(
    ...
    # M = 1
    acc = tl.zeros([BLOCK_N, 1], dtype=tl.float32) # 128 x 1

    for k in range(0, K, BLOCK_K):
        a_ptrs = A_ptr + (k + offs_k[None,:]) * stride_ak     # shape [BLOCK_K]
        b_ptrs = B_ptr + (k + offs_k[None, :]) * stride_bk + offs_n[:, None] * stride_bn  # shape [BLOCK_K, BLOCK_N]

        A_vals = tl.load(a_ptrs, mask=(k + offs_k[None,:] < K), other=0.0)             # [BLOCK_K]
        B_vals = tl.load(b_ptrs, mask=(k + offs_k[None, :]< K) & (offs_n[:, None]< N), other=0.0)  # [BLOCK_K, BLOCK_N]

        v = A_vals * B_vals
        acc += tl.sum(v, axis=1, keep_dims=True)  # [BLOCK_N]

    c_ptrs = C_ptr + offs_n[:, None] * stride_cn
    tl.store(c_ptrs, acc, mask=offs_n[:, None] < N)
    
@triton.jit
def matmul_add_1xK_KxN_kernel(
    ...
    # Load bias and add
    bias_ptr = Bias_ptr + offs_n[:, None]
    bias = tl.load(bias_ptr, mask=offs_n[:, None] < N, other=0.0)
...
def triton_matmul_fp32(A: torch.Tensor, B: torch.Tensor):
    ...
    assert (A.dtype == B.dtype and (B.dtype == torch.float32 or B.dtype == torch.float16))
    M, K = A.shape
    K2, N = B.shape
    assert K == K2
...
def triton_matmul_add_fp32(A: torch.Tensor, B: torch.Tensor, bias: torch.Tensor):
    ...
    if A.shape[0] == 1:
        ...
        matmul_add_1xK_KxN_kernel[grid](
            A, B, bias, C,
            K, N,
            A.stride(1),
            B.stride(0), B.stride(1),
